...
\begin{equation}
p(\bx, \bl) = \prod_t p(x_t,l_t\mid\bx_{<t}) = \prod_t p(x_t\mid l_t,\bx_{<t})p(l_t\mid\bx_{<t}).
\end{equation}
\subsection{$p(x_t\mid l_t,\bx_{<t})$}
The first term takes the form a language model,
where the distribution over the next token
$$p(x_t\mid l_t,\bx_{<t}) = f([\bh_{l_t:t-1},h_t])$$
